import { toRefs, onMounted, onUnmounted, watch, defineComponent, ref, renderSlot, useSlots, createVNode, reactive, computed, onBeforeUnmount, nextTick, onBeforeMount, Transition, mergeProps, unref, withModifiers, Comment, Text, Fragment, h, inject, withDirectives, cloneVNode, provide, Teleport, vShow, createTextVNode, isVNode } from "vue";
import "clipboard";
...
ToolBarHandler.undo = (editor) => {
  editor.undo();
};
ToolBarHandler.redo = (editor) => {
  editor.redo();
};
ToolBarHandler.bold = (editor) => {
...
ToolBarHandler.underline = (editor) => {
  const cursor = editor.getCursor();
  const selection = editor.getSelection();
  editor.focus();
...
ToolBarHandler.checkList = (editor) => {
  const selection = editor.getSelection();
  editor.focus();
  if (selection === "") {
    editor.replaceSelection("[ ] " + selection);
...
ToolBarHandler.image = (editor, params) => {
...
ToolBarHandler.file = (editor, params) => {
  const stopEventPropagation = (e) => {
    e.stopPropagation();
  };
...
ToolBarHandler.fullscreen = (editor) => {
  editor.focus();
};
    ...
    return () => createVNode(Fragment, null, [createVNode(PopperTrigger, null, {
      ...
      default: () => [createVNode(Transition, {
        ...
        default: () => [createVNode(FlexibleOverlay, {
          ...
          default: () => {
            var _a;
            return [slots.content ? (_a = slots.content) == null ? void 0 : _a.call(slots) : createVNode("span", null, [content.value])];
          }
        })]
      })]
    })]);
