...
var devuiGreenDarkTheme = new Theme({
...
var devuiCustomComponentsThemeData = {
...
var devuiCustomComponentsDarkThemeData = {
  "devui-btn-sm-padding": "0 12px",
  "devui-btn-padding": "0 16px",
  "devui-btn-lg-padding": "0 20px",
...
for (model in convert$2) {
  if (convert$2.hasOwnProperty(model)) {
    ...
    channels = convert$2[model].channels;
    labels = convert$2[model].labels;
...
var color = Color;
var CustomThemeUtils = class _CustomThemeUtils {
  static genThemeData(colorChanges, isDark = false, effect, options = {
    ...
    const pattern = _CustomThemeUtils.genColorPattern(options.colorHierarchy, themeData);
    const updatedPattern = _CustomThemeUtils.updateColor(colorChanges, pattern, effect);
    _CustomThemeUtils.fillEmptyColor(updatedPattern, effect);
    return _CustomThemeUtils.pattern2ThemeData(updatedPattern);
  }
  static genColorPattern(colorHierarchy, themeData) {
    const pattern = {};
    const offset = _CustomThemeUtils.getThemeOffset(colorHierarchy, themeData);
    offset.forEach((obj) => {
  ...
  static getThemeOffset(colorHierarchy, themeData) {
    const colorKeys = Object.keys(colorHierarchy);
    const themeColorOffset = colorKeys.map(
      (key) => ({
        colorName: key,
      ...
      if (colorObj.extends) {
        colorObj.offset = {
          hsl: _CustomThemeUtils.getColorOffset(colorObj.color, colorObj.extends, "hsl"),
          hsv: _CustomThemeUtils.getColorOffset(colorObj.color, colorObj.extends, "hsv")
        };
      }
      return colorObj;
    });
  ...
  static getColorOffset(target, source, mode) {
    ...
    const percent = {
      [key[1] + "p"]: offset[key[1]] > 0 ? offset[key[1]] * 100 / (100 - sourceModel.color[1]) : offset[key[1]] * 100 / sourceModel.color[1],
      [key[2] + "p"]: offset[key[2]] > 0 ? offset[key[2]] * 100 / (100 - sourceModel.color[2]) : offset[key[2]] * 100 / sourceModel.color[2],
  ...
  static updateColor(colorChanges, colorHierarchy, effect) {
    const changeKeys = colorChanges.map((change) => change.colorName);
    const changeStack = [...changeKeys];
    const colorKeys = Object.keys(colorHierarchy);
    const pattern = JSON.parse(JSON.stringify(colorHierarchy));
    let count = 0;
    while (changeStack.length) {
      ...
      if (count < changeKeys.length) {
        ...
        const { mode, offset } = _CustomThemeUtils.getColorEffectOffset(extendsColor, colorOffset, effect);
        pattern[handleKey]["default-value"] = _CustomThemeUtils.getHexOrRgba(_CustomThemeUtils.getColorValue(extendsColor, offset, mode));
      }
      colorKeys.forEach((colorName2) => {
        if (handleKey === pattern[colorName2].extends) {
          if (!(changeStack.indexOf(colorName2) > -1) && !(changeKeys.indexOf(colorName2) > -1)) {
            changeStack.push(colorName2);
          }
        }
      });
      count++;
  ...
  static fillEmptyColor(pattern, effect) {
    const colorKeys = Object.keys(pattern);
    const noColorArray = colorKeys.map((colorName2) => ({
      colorName: colorName2,
      pattern: pattern[colorName2]
    })).filter((color2) => !color2.pattern["default-value"]);
    noColorArray.forEach((color$1) => {
      const handleKey = color$1.colorName;
      const extendsKey = pattern[handleKey].extends;
      const extendsColor = color(pattern[extendsKey]["default-value"]);
      const colorOffset = pattern[handleKey].offset;
      const { mode, offset } = _CustomThemeUtils.getColorEffectOffset(extendsColor, colorOffset, effect);
  ...
  static getColorEffectOffset(source, colorOffset, effect) {
    ...
    switch (effect) {
      ...
      case "strong":
        result = {
          ...
          offset: {
            ...colorOffset.hsl,
            sp: ((_a = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _a.sp) > 0 ? _CustomThemeUtils.minmax(((_b = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _b.sp) * 1.3, (_c = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _c.sp, 100) : ((_d = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _d.sp) * 0.75
          }
        };
        break;
      case "soft":
        result = {
          ...
          offset: {
            ...colorOffset.hsv,
            sp: ((_e = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _e.sp) > 0 ? _CustomThemeUtils.minmax(((_f = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _f.sp) * 1.25, (_g = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _g.sp, 100) : ((_h = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _h.sp) * 0.5
          }
        };
        break;
      case "light":
        result = {
          ...
          offset: {
            ...colorOffset.hsl,
            lp: ((_i = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _i.lp) > 0 ? _CustomThemeUtils.minmax((_j = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _j.lp, (_k = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _k.lp, 100) : ((_l = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _l.lp) * 0.2
          }
        };
        break;
      case "contrast":
  ...
  static pattern2ThemeData(pattern) {
    const themeData = {};
    const colorKeys = Object.keys(pattern);
    colorKeys.forEach((colorName2) => {
      themeData[colorName2] = pattern[colorName2]["default-value"];
    });
    return themeData;
  }
  static getColorValue(source, offset, mode) {
    const sourceModel = source[mode]();
    const key = mode.split("");
    const value = {
      [key[0]]: (sourceModel.color[0] + offset[key[0]]) % 360,
      [key[1]]: offset[key[1] + "p"] > 0 ? sourceModel.color[1] + offset[key[1] + "p"] * (100 - sourceModel.color[1]) / 100 : sourceModel.color[1] + sourceModel.color[1] * offset[key[1] + "p"] / 100,
      [key[2]]: offset[key[2] + "p"] > 0 ? sourceModel.color[2] + offset[key[2] + "p"] * (100 - sourceModel.color[2]) / 100 : sourceModel.color[2] + sourceModel.color[2] * offset[key[2] + "p"] / 100,
      a: offset.ap > 0 ? sourceModel.valpha + offset.ap * (1 - sourceModel.valpha) / 100 : sourceModel.valpha + sourceModel.valpha * offset.ap / 100
    };
    return color([value[key[0]], value[key[1]], value[key[2]], value.a], mode);
  }
  static getHexOrRgba(color2) {
    if (color2.alpha() < 1) {
      const rgb = color2.rgb();
...
var _PrefersColorSchemeMediaQuery = class {
  constructor() {
...
var PrefersColorSchemeMediaQuery = _PrefersColorSchemeMediaQuery;
...
devui-theme/index.es.js:
   ...
   *)
*/
//# sourceMappingURL=devui-theme.js.map
